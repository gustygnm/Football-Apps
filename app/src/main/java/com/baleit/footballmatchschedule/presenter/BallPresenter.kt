...
class BallPresenter(
    ...
    fun getTeamListNext(league: String?) {
        view.showLoading()
        ...
        async(context.main) {
            val data = bg {
                gson.fromJson(apiRepo.doRequest(ApiCall.getNextEvent(league)),
    ...
    fun getTeamListPrev(league: String?) {
        view.showLoading()
        ...
        async(context.main) {
            val data = bg {
                gson.fromJson(apiRepo
                        .doRequest(ApiCall.getPrevEvent(league)),
                        BallResponse::class.java
                )
            }
            view.showMatch(data.await().matchItems)
